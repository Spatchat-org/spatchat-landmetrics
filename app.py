...
# --- On file upload ---
def on_file_upload(file, history):
    fig = preview_raster(file)
    new_history = history + [{"role":"assistant","content":"Awesome! I can see your raster now! You can ask about CRS, resolution, extent, or calculate any metric."}]
    return fig, new_history
...
if glossary is None:
    glossary = {
        ...
        # Add more minimal defaults or rebuild full glossary here
    }
metric_definitions = {code: info for code, info in glossary.items()}
synonyms = {}
for code, info in glossary.items():
    name_lower = info.get('name', code).lower()
    phrase = code.replace('_', ' ')
    synonyms[code] = [code.lower(), phrase, name_lower]
...
def list_metrics():
    categories = {
        "Landscape": [k for k,v in metric_definitions.items() if 'Landscape' in v.get('level','')],
        "Class":     [k for k,v in metric_definitions.items() if 'Class'     in v.get('level','')],
        "Patch":     [k for k,v in metric_definitions.items() if 'Patch'     in v.get('level','')]
    }
    lines = []
    for level, keys in categories.items():
        lines.append(f"**{level}-level metrics:**")
        for k in keys:
            info = metric_definitions[k]
            lines.append(f"- {info['name']} (`{k}`): {info['definition']}")
    return "\n".join(lines)
...
def compute_metric(file, key):
    ...
    return meta + land_part + f"**Class-level {name}:**\n{table}"
...
with gr.Blocks(title="Spatchat") as iface:
    ...
    file_input.change(on_file_upload, inputs=[file_input, chatbot], outputs=[raster_output, chatbot])
